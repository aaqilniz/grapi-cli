...
export default class ExternalOperation extends Command {
    ...
    async run() {
        ...
        for (let i = 0; i < optionsArray.length; i++) {
            ...
            const { ds, url, headers, apiFunction, type, controller, } = options;
            let { method, pathParams, bodyParams, queryParams, name, apiUri, spliter } = options;
            if (!spliter && !apiUri)
                spliter = 'v1';
            if (spliter)
            ...
            let path = '';
            let functionParams = '';
            queryParamList.forEach((key) => { query += `${key}: '{${key}}', `; });
            // if (bodyParamList.length) body = `body: '{body}', `;
            bodyParamList.forEach((key) => { body += `${key}: '{${key}}', `; });
            if (pathKey)
                path = `${pathKey}: '{${pathKey}}'`;
            queryParamList.forEach((key) => { functionParams += `'${key}',`; });
            if (bodyParamList.length) {
                bodyParamList.forEach((key) => { functionParams += `'${key}',`; });
            }
