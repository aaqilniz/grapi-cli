...
const patches: Patch = {
    ...
    addPrefix: {
        ...
        appendPrefix: {
            ...
            replacement: `this.controllerSpecs = this.controllerSpecs.map(c => {if (c.tag.includes(this.options.prefix)) {const splited = c.tag.split(this.options.prefix);if(splited.length === 2) {c.tag = splited[1];} else {c.tag = splited[1] + splited[2];}}return c;});const choices = this.controllerSpecs.map(c => {`,
            path: '/generators/openapi/index.js'
        },
