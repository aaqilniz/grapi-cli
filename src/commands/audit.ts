...
export default class Audit extends Command {
  ...
  static override description = 'generate audit artifacts.'
  ...
  public async run(): Promise<void> {
    ...
    if (!auditDS) {
      const executed: any = await execute(command, 'generating audit datasource.');
    ...
    if (!auditModel) {
      ...
      const executed: any = await execute(discoverAuditModel, 'discovering audit model.');
      if (executed.stderr) console.log(chalk.bold(chalk.green(executed.stderr)));
      if (executed.stdout) console.log(chalk.bold(chalk.green(executed.stdout)));
    ...
    if (!auditRepo) {
      ...
      const executed: any = await execute(repoCommand, 'generating audit repo.');
      if (executed.stderr) console.log(chalk.bold(chalk.green(executed.stderr)));
      if (executed.stdout) console.log(chalk.bold(chalk.green(executed.stdout)));
