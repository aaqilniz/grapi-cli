...
import { ConstructorDeclaration, MethodDeclaration, ObjectLiteralExpression, Project, PropertyAssignment, Scope, SyntaxKind } from 'ts-morph';
import fs from 'fs'
...
export default class Cache extends Command {
  ...
  public async run(): Promise<void> {
    ...
    valueMethodText = valueMethodText
      ?.replace(
        ...
        `if (this.${redisDS.toLowerCase()}.name === this.metadata.datasource) {
        customRepo = new CustomRepo(this.${redisDS.toLowerCase()});
      }`);
    if (valueMethodText) valueMethod?.replaceWithText(valueMethodText);
    ...
    const excludingObjects: any = {};
    const includingObjects: any = {};
        ...
        if (this.isValidMethod(text)) {
          methods.forEach(method => {
            ...
            for (let j = 0; j < includings.length; j++) {
              ...
              if (including.includes('*')) { including = including.replaceAll('*', '.*'); }
            ...
            for (let j = 0; j < excludings.length; j++) {
              let excluding: string = excludings[j];

              if (excluding.includes('*')) { excluding = excluding.replaceAll('*', '.*'); }
  ...
  private addCacheDecorator(addDecoratorTo: MethodDeclaration, args: string[]): void {
    addDecoratorToMethod(addDecoratorTo, 'cache', args)
  }
  private addStatements(
    addStatementsTo: ConstructorDeclaration,
    statementsPosition: number,
    statements: string[],
  ): void {
  ...
  private constructRegex(input: string): any {
    let regexString = '';
